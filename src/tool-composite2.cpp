...
// Filename:      tool-composite2.cpp
// URL:           https://github.com/craigsapp/humlib/blob/master/src/tool-composite2.cpp
// Syntax:        C++11; humlib
...
//
// To do:
// (3) debugging of coincidence rhythm
...
/////////////////////////////////
//
// Tool_composite2::Tool_composite2 -- Set the recognized options for the tool.
//

Tool_composite2::Tool_composite2(void) {
...
//////////////////////////////
//
// Tool_composite2::initialize -- Prepare interface variables.
//

void Tool_composite2::initialize(HumdrumFile& infile) {
...
//////////////////////////////
//
// Tool_composite2::initializeNumericAnalyses --
//

void Tool_composite2::initializeNumericAnalyses(HumdrumFile& infile) {
	double initValue = 0;
	m_analyses.clear();
...
/////////////////////////////////
//
// Tool_composite2::run -- Do the main work of the tool.
//

bool Tool_composite2::run(HumdrumFileSet& infiles) {
	bool status = true;
...
bool Tool_composite2::run(const string& indata, ostream& out) {
	HumdrumFile infile;
...
bool Tool_composite2::run(HumdrumFile& infile, ostream& out) {
	bool status = run(infile);
	if (hasAnyText()) {
		getAllText(out);
	} else {
...
bool Tool_composite2::run(HumdrumFile& infile) {
	processFile(infile);
	return true;
}
...
//////////////////////////////
//
// Tool_composite2::hasGroupInterpretations --
//

bool Tool_composite2::hasGroupInterpretations(HumdrumFile& infile) {
...
//////////////////////////////
//
// Tool_composite2::prepareOutput --
//

void Tool_composite2::prepareOutput(HumdrumFile& infile) {
...
//////////////////////////////
//
// Tool_composite2::generateVerseLabelLine --
//

string Tool_composite2::generateVerseLabelLine(HumdrumFile& output, HumdrumFile& input, int line) {
	string outstring;
	string inputBlanks;
...
//////////////////////////////
//
// Tool_composite2::generateStriaLine --
//

string Tool_composite2::generateStriaLine(HumdrumFile& output, HumdrumFile& input, int line) {
	string outstring;
	string inputBlanks;
	for (int i=0; i<input[line].getFieldCount(); i++) {
		inputBlanks += "*";
...
//////////////////////////////
//
// Tool_composite2::getAnalysisOutputLine --
//

void Tool_composite2::getAnalysisOutputLine(ostream& output, HumdrumFile& infile, int line) {
	...
	if (m_coincidenceQ) {
		output << getCoincidenceToken(infile, line);
		if (processedQ) {
			output << "\t";
		}
		processedQ = true;
		...
		if (m_numericAnalysisSpineCount) {
			addNumericAnalyses(output, infile, line, m_analyses[m_COINCIDENCE]);
		}
	}

	if (m_fullCompositeQ) {
		output << getFullCompositeToken(infile, line);
		if (processedQ) {
			output << "\t";
		}
		processedQ = true;
		if (m_numericAnalysisSpineCount) {
			addNumericAnalyses(output, infile, line, m_analyses[m_COMPOSITE_FULL]);
		}
	}
	...
	if (m_groupsQ) {
		...
		processedQ = true;
		output << getGroupCompositeToken(infile, line, 0);
		if (m_numericAnalysisSpineCount) {
			addNumericAnalyses(output, infile, line, m_analyses[m_COMPOSITE_A]);
		}
		output << "\t";
		output << getGroupCompositeToken(infile, line, 1);
		if (m_numericAnalysisSpineCount) {
			addNumericAnalyses(output, infile, line, m_analyses[m_COMPOSITE_B]);
		}
	}
}
...
//////////////////////////////
//
// Tool_composite2::addNumericAnalyses -- For a composite rhythm spine, add numeric
//     analyses spines after it as necessary.
//

void Tool_composite2::addNumericAnalyses(ostream& output, HumdrumFile& infile, int line,
...
//////////////////////////////
//
// Tool_composite2::getCoincidenceToken --
//

string Tool_composite2::getCoincidenceToken(HumdrumFile& infile, int line) {
...
//////////////////////////////
//
// Tool_composite2::getFullCompositeToken --
//

string Tool_composite2::getFullCompositeToken(HumdrumFile& infile, int line) {
	if (infile[line].isData()) {
		if (m_fullComposite[line] != "") {
			return m_fullComposite[line];
		} else {
			return ".";
		}
	} else if (infile[line].isInterpretation()) {
...
//////////////////////////////
//
// Tool_composite2::getGroupCompositeToken --
//    group 0 == "A"
//    group 1 == "B"
//

string Tool_composite2::getGroupCompositeToken(HumdrumFile& infile, int line, int group) {
	string tgroup = (group == 0) ? "A" : "B";
	if (infile[line].isData()) {
		if (m_groups.at(group).at(line) != "") {
			return m_groups.at(group).at(line);
		} else {
			return ".";
		}
	} else if (infile[line].isInterpretation()) {
...
//////////////////////////////
//
// Tool_composite2::getTimeSignature --
//

string Tool_composite2::getTimeSignature(HumdrumFile& infile, int line, const string& group) {
...
//////////////////////////////
//
// Tool_composite2::getMeterSymbol --
//

string Tool_composite2::getMeterSymbol(HumdrumFile& infile, int line, const string& group) {
	if (!infile[line].isInterpretation()) {
		return "*";
	}
	HTp lastToken = NULL;
...
//////////////////////////////
//
// Tool_composite2::analyzeFullCompositeRhythm --
//

void Tool_composite2::analyzeFullCompositeRhythm(HumdrumFile& infile) {
...
void Tool_composite2::analyzeGroupCompositeRhythms(HumdrumFile& infile) {
	vector<HumNum> durations(infile.getLineCount());
	for (int i=0; i<infile.getLineCount(); i++) {
		durations[i] = infile[i].getDuration();
	}
...
void Tool_composite2::getGroupRhythms(vector<vector<string>>& rhythms,
...
void Tool_composite2::getGroupRhythms(vector<string>& rhythms, vector<HumNum>& durs,
...
//////////////////////////////
//
// Tool_composite2::getGroupDurations --
//

void Tool_composite2::getGroupDurations(vector<vector<HumNum>>& groupdurs,
...
void Tool_composite2::getGroupDurations(vector<HumNum>& groupdurs,
...
/////////////////////////////
//
// Tool_composite2::getGroupStates -- Pull out the group note states for each
...
void Tool_composite2::getGroupStates(vector<vector<int>>& groupstates, HumdrumFile& infile) {
...
//////////////////////////////
//
// Tool_composite2::getLineDuration -- Return the duration of the line, but return
...
HumNum Tool_composite2::getLineDuration(HumdrumFile& infile, int index, vector<bool>& isNull) {
	if (isNull[index]) {
		return 0;
	}
...
//////////////////////////////
//
// Tool_composite2::assignGroups -- Add a parameter
...
void Tool_composite2::assignGroups(HumdrumFile& infile) {
...
//////////////////////////////
//
// Tool_composite2::backfillGroup -- Go back and reassign a group to all lines
...
void Tool_composite2::backfillGroup(vector<vector<string>>& curgroup, HumdrumFile& infile,
...
//////////////////////////////
//
// Tool_composite2::analyzeLineGroups -- Look at each line for Group A and B and determine if
...
void Tool_composite2::analyzeLineGroups(HumdrumFile& infile) {
	for (int i=0; i<infile.getLineCount(); i++) {
		if (!infile[i].hasSpines()) {
			continue;
		}
...
//////////////////////////////
//
// Tool_composite2::analyzeLineGroup --
//
//     9 = TYPE_UNDEFINED           = "undefined"
//     3 = TYPE_NoteSustainAttack   = "snote"
//     2 = TYPE_NoteAttack          = "note"
...
void Tool_composite2::analyzeLineGroup(HumdrumFile& infile, int line, const string& target) {
...
//////////////////////////////
//
// Tool_composite2::extractGroup --
//

void Tool_composite2::extractGroup(HumdrumFile& infile, const string &target) {
	for (int i=0; i<infile.getLineCount(); i++) {
		if (!infile[i].isData()) {
...
//////////////////////////////
//
// Tool_composite2::getGroupNoteType --
//
//  9 = TYPE_UNDEFINED
...
int Tool_composite2::getGroupNoteType(HumdrumFile& infile, int line, const string& group) {
	if (!infile[line].isData()) {
		return TYPE_NONE;
	}
...
//////////////////////////////
//
// Tool_composite2::printGroupAssignments -- for debugging of group assignments.
//

void Tool_composite2::printGroupAssignments(HumdrumFile& infile) {
	for (int i=0; i<infile.getLineCount(); i++) {
		if (!infile[i].hasSpines()) {
			cerr << infile[i] << endl;
			continue;
...
//////////////////////////////
//
// Tool_composite2::typeStringToInt -- Convert between numeric and string state forms.
//

int Tool_composite2::typeStringToInt(const string& value) {
...
//////////////////////////////
//
// Tool_composite2::doNumericAnalyses --
//

void Tool_composite2::doNumericAnalyses(HumdrumFile& infile) {
	if (m_analysisOnsetsQ) {
...
//////////////////////////////
//
// Tool_composite2::doOnsetAnalyses -- targetGroup == "" means full composite onsets.
//
// First index of m_analyses is rhythm type, and second index is analysis type, with "0" being onsets.
//

void Tool_composite2::doOnsetAnalyses(HumdrumFile& infile) {
...
//////////////////////////////
//
// Tool_composite2::doOnsetAnalysis -- Count all of the notes on each line
//   (that are in **kern spines, ignoring other kern-like spines).
//

void Tool_composite2::doOnsetAnalysis(vector<double>& analysis, HumdrumFile& infile,
...
//////////////////////////////
//
// Tool_composite2::countNoteOnsets --
//

int Tool_composite2::countNoteOnsets(HTp token) {
...
//////////////////////////////
//
// Tool_composite2::doAccentAnalyses --
//

void Tool_composite2::doAccentAnalyses(HumdrumFile& infile) {
...
//////////////////////////////
//
// Tool_composite2::doOrnamentAnalyses --
//

void Tool_composite2::doOrnamentAnalyses(HumdrumFile& infile) {
...
//////////////////////////////
//
// Tool_composite2::doSlurAnalyses --
//

void Tool_composite2::doSlurAnalyses(HumdrumFile& infile) {
...
//////////////////////////////
//
// Tool_composite2::doTotalAnalyses -- The totals analysis is the last
//    index in the m_analysis array.  It is the sum of all other analyses
//    (provided that they are non-negative).
//

void Tool_composite2::doTotalAnalyses(HumdrumFile& infile) {
...
//////////////////////////////
//
// Tool_composite2::analyzeOutputVariables --
//

void Tool_composite2::analyzeOutputVariables(HumdrumFile& infile) {
